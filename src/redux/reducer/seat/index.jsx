  ...
  customer: [
    {
      ...
      total: 0,
      seat: [
        // { seatName: "J1", gia: 75000 },
        // { seatName: "J3", gia: 75000 },
        // { seatName: "J4", gia: 75000 },
      ]
    }
  ]

}
const seatReducer = (state = initialSeat, action) => {
  switch (action.type) {
    case "BOOK_TICKET":
      return {
        ...state,
        ironMan: state.ironMan.map((hangGhe, index) => {
          return index === action.payload.rowIndex ?
            {
              ...hangGhe,
              danhSachGhe: state.ironMan[index].danhSachGhe.map((ghe, gheIndex) => {
                return gheIndex === action.payload.colIndex ?
                  { ...ghe, daDat: action.payload.booked } :
                  ghe
              })
            } : hangGhe
        }
        ),
        customer: state.customer.map((cus, indexCus) => {
          const indexSeatInCustomer = state.customer[indexCus].seat.findIndex((item) => item.seatName === action.payload.seatName)

          return indexCus === 0 ?
            {
              ...cus,
              // total: action.payload.booked ?
              //   state.customer[indexCus].total + 1 :
              //   state.customer[indexCus].total - 1,
              seat: indexSeatInCustomer === -1 ? [
                ...state.customer[indexCus].seat,
                { seatName: action.payload.seatName, price: action.payload.price },
              ] :

                state.customer[indexCus].seat.filter((item, index) => {
                  return index !== indexSeatInCustomer
                })



              // state.customer[indexCus].seat.map((item, indexItem) => {
              //   return indexItem === indexSeatInCustomer ?
              //     { ...item, seatName }
              // })
      ...
      return {
        ...
        customer: state.customer.map((cus, indexCus) => {
          return indexCus === 0 ?
            {
              ...cus,
              customerName: action.payload.fullName,
